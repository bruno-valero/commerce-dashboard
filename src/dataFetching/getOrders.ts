...
export default async function getOrders({ baseURL, init, registeredDomains }:GetOrdersPropsType):Promise<InsertRegisteredDomainsReturnType<OrdersDataItemType>> {
  const input:FetchAuthInput = baseURL + '/api/orders';
  const response = await fetchAuthJson({ input, init, registeredDomains}) as Promise<InsertRegisteredDomainsReturnType<OrdersDataItemType>>;
  return response;
}
