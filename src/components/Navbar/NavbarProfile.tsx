...
export default function NavbarProfile({ title, icon, color, dotColor, customFunc, style }:NavbarProfileProps) {
  return (
    <ToolTipClientComponent content={title} position='BottomCenter' >
      <div onClick={customFunc} style={style} className={`${color ?? ''} relative flex items-center gap-2 cursor-pointer p-1 hover:bg-gray-300 rounded-lg`} >
